...
impl ViewClass for TimeSeriesView {
    ...
    fn ui(
        ...
        // all panels.
        //
        // egui_plot 0.35 computes axis size as
        //   `max(min_thickness, measured_label_size_last_frame)`.
        // For uniform widths on Y across stacked panels we pick a
        // value comfortably above the widest label ANY panel will
        // render. The same constant gets misused for X-axis HEIGHT
        // upstream, but X labels are one-line short ("14:30") and
        // need only ~24 px — so we split into two constants here.
        //
        // - Y (vertical-axis WIDTH): 110 px fits "−60,000" + margin.
        // - X (horizontal-axis HEIGHT): 24 px fits "HH:MM" + tick.
        //   Previously 110 px which left a huge dead zone below each
        //   chart — that was the "large bottom padding on accel mag
        //   and HR" the user reported.
        let min_y_axis_thickness = 110.0_f32;
        let min_x_axis_thickness = 24.0_f32;
        ...
        ui.scope(|ui| {
            ...
            let mut plot = Plot::new(plot_id_src)
                ...
                // attempting a horizontal scrub, which then drifts
                // the y-axis off our blueprint range. Gating drag on
                // the same `zoom_lock` makes the lock feel cohesive
                // ("I locked this axis"); the user can still scroll
                // & pan freely on the unlocked axis (X).
                .allow_drag(!zoom_lock)
                ...
                .custom_x_axes(vec![
                    egui_plot::AxisHints::new_x()
                        ...
                        .formatter({
                            ...
                            // panels share the linked X axis). The
                            // upstream `DateVisibility::HideDate`
                            // knob already exists for this exact
                            // case — the tooltip formatter at
                            // L1349 uses it; we wire it into the
                            // tick labels too. With Hide, ticks fall
                            // back to HH:MM / HH:MM:SS depending on
                            // zoom level. Whole-recording date stays
                            ...
                            move |time, _| {
                                ...
                                .format_compact(no_date_format)
                            }
                        }),
                ])
                .custom_y_axes(vec![
                    egui_plot::AxisHints::new_y()
                        .min_thickness(min_y_axis_thickness)
            ...
            // plot, and the bottom scrubber only controls the L1
            // plots, not the accel magnitude". With link_axis on X
            // (false on Y so we still keep independent y-ranges),
            // any pan/zoom propagates to every panel on the same
            // timeline. The bottom time-panel scrubber also feeds
            // into this shared viewport, so all four panels follow
            // it together.
            ...
            if is_resetting {
                ...
                if unchanged_bounds != *transform.bounds() {
                    ...
                    // back egui_plot's internal post-render bounds.
                    // Without this gate, every frame's tiny rounding/
                    // padding drift between (y_range we set) and
                    // (transform.bounds() egui returns) gets
                    // persisted as the new y_range, then drifts again
                    // next frame — visible as the y-axis labels
                    // shifting up/down as the user scrolls the time
                    // window. zoom_lock=true was supposed to prevent
                    // this; with this gate it actually does.
                    if new_y_range != y_range && !zoom_lock.y {
...
fn paint_time_cursor(
    ...
    // just a reference indicator showing where the current time falls
    // in that panel's X axis. Dropping the triangle eliminates the
    // "multiple scrubber flags competing for attention" effect the
    // user reported when stacking 7 TimeSeriesView panels vertically.
    //
    // We keep hover/drag interaction (the line still grabs the
    // resize cursor + supports click-drag for time scrub), only the
    // VISUAL is muted.
    ...
    let stroke_color = if is_being_dragged {
        ...
        // Inactive state: ~40 % alpha mid-gray. Barely visible when the
    ...
    ui.painter().vline(time_x, response.rect.y_range(), stroke);
}
