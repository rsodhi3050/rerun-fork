...
impl VisualizerSystem for SeriesLinesSystem {
    ...
    fn execute(
        ...
        // Sequential — load_series touches RefCell-backed viewer state
        // (blueprint resolution + fallback queries) and rayon-parallel
        // invocations from multiple workers double-borrow the cell,
        // panicking with `panic_already_borrowed` inside execute.
        // Per-entity load is microseconds; serializing has no
        // perceptible cost on real workloads (~10 entities per view).
