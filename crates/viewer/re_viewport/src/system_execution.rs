...
fn run_view_systems(
...

    {
        ...
        // Sequential — every context system here shares `&view_ctx`,
        // which transitively owns RefCells (blueprint resolver,
        // chunk-store query cache, fallback registry). Running them
        // in parallel via `par_iter_mut` triggers
        // `panic_already_borrowed` from cross-thread RefCell racing.
        // Each system runs in microseconds; the perceived cost of
        // serializing is zero on realistic workloads.
        context_systems
            .systems
            .iter_mut()
    ...
    // Sequential — same reason as ViewContextSystem above. All
    // visualizers in a view share `&view_ctx`, including its
    // RefCell-backed query cache. par_iter here is the root cause
    // of `SeriesLinesSystem::execute` panicking with
    // `panic_already_borrowed` when more than one TimeSeriesView
